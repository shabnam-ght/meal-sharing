...
router.get("/", async (request, response) => {
  try {
    if (request.query.limit && request.query.maxPrice){
      ...
      const limitedMeals=await knex("meals").where("price","<=",maxPrice).limit(limit);
    ...
    response.json(limitedMeals)
      ...
      const matchingTitles=await knex("meals").where("title","like",`%${title}%`)
    response.json(matchingTitles)  

    }else if(request.query.createdAfter){
      const createdAfter=request.query.createdAfter;
      const datescreatedAfter=await knex("meals").where("created_date",">",createdAfter)
...
router.put("/:id", async (request, response) => {
  try {
    const meal = await knex("meals").where("id",request.params.id).update(request.body);
    response.json(meal);
  } catch (error) {
    throw error;
  }
